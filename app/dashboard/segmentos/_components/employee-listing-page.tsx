    ...
    <PageContainer scrollable>
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          <Heading
            title={`Segmentos (${filteredPlans?.length})`}
            description="Gerenciar Segmentos."
          />

          <Link
            href={'/dashboard/segmentos/novo'}
            className={cn(buttonVariants({ variant: 'default' }))}
          >
            <Plus className="mr-2 h-4 w-4" /> Add Novo Segmento
          </Link>
        </div>
        <Separator />

        {/* Conditional rendering based on loading or error */}
        {loading && <p>Carregando os Segmentos...</p>}
        {updateFlag && <p></p>}
        {error && <p>Erro ao carregar os Segmentos</p>}
